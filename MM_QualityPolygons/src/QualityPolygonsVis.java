...
public class QualityPolygonsVis {
	...
	String validatePoly(int[] poly, int n) {
		...
		if (100 * 100 * minLen2 < maxLen2 * sq(100 - sidesDiff)) {
			...
			return "sides of polygon must be approximately equal; here minLen = " + minLen + ", maxLen = " + maxLen + ", diff = "
		...
		if (100 * 100 * minR2 < maxR2 * sq(100 - radiiDiff)) {
			...
			return "polygon must be approximately inscribed; here minR = " + minLen + ", maxR = " + maxLen + ", diff = "
					+ (maxLen - minLen) / maxLen + ".";
		}
	...
	public double runTest(String seed) {
		try {
			...
			if (true || proc != null) {
				...
				String[] ret;
				try {
					ret = new QualityPolygons().choosePolygons(pointsPar, sidesDiff, radiiDiff);
				} catch (Exception e) {
	...
	// ---------------------------------------------------
	String[] choosePolygons(int[] points, int sidesDiff, int radiiDiff) throws IOException { //pass the params to the solution and get the return
		int i;
		...
		public void paint(Graphics g) {
			try {
				...
				for (i = 0; i < polys.length; i++) {
					...
					for (j = 0; j < n; j++)
						g2.drawLine(p[polys[i][j]].x, (SZ - 1 - p[polys[i][j]].y), p[polys[i][(j + 1) % n]].x, (SZ - 1 - p[polys[i][(j + 1)
								% n]].y));
				}
				//draw current poly
